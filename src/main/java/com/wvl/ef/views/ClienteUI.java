...
public class ClienteUI extends JFrame {
    ...
    public ClienteUI() {
        ...
        JLabel l1 = new JLabel("Nombres"); l1.setBounds(20,20,100,25);
        JLabel l2 = new JLabel("Apellidos"); l2.setBounds(20,50,100,25);
        JLabel l3 = new JLabel("Email"); l3.setBounds(20,80,100,25);
        JLabel l4 = new JLabel("Telefono"); l4.setBounds(20,110,100,25);

        txtNom = new JTextField(); txtNom.setBounds(120,20,200,25);
        txtApe = new JTextField(); txtApe.setBounds(120,50,200,25);
        txtEmail = new JTextField(); txtEmail.setBounds(120,80,200,25);
        txtTel = new JTextField(); txtTel.setBounds(120,110,200,25);

        JButton btnAgregar = new JButton("Agregar"); btnAgregar.setBounds(350,20,120,25);
        JButton btnEditar  = new JButton("Editar");  btnEditar.setBounds(350,55,120,25);
        JButton btnEliminar= new JButton("Eliminar");btnEliminar.setBounds(350,90,120,25);
        JButton btnRefrescar = new JButton("Refrescar"); btnRefrescar.setBounds(350,125,120,25);
        ...
        add(txtNom); add(txtApe); add(txtEmail); add(txtTel);
        ...
        btnEliminar.addActionListener(e -> {
            ...
            int id = Integer.parseInt(modelo.getValueAt(fila,0).toString());
            int resp = JOptionPane.showConfirmDialog(this, "¿Seguro que deseas eliminar el cliente seleccionado?", "Confirmar", JOptionPane.YES_NO_OPTION);
        ...
        btnRefrescar.addActionListener(e -> { limpiarCampos(); cargarTabla(); });
    }

    private String validarCampos(String nombres, String apellidos, String email, String telefono) {
        if (nombres == null || nombres.isEmpty() || nombres.length() < 2) return "El campo 'Nombres' es obligatorio (mín 2 caracteres).";
        if (apellidos == null || apellidos.isEmpty() || apellidos.length() < 2) return "El campo 'Apellidos' es obligatorio (mín 2 caracteres).";
        if (email == null || email.isEmpty()) return "El campo 'Email' es obligatorio.";
        if (!EMAIL_PATTERN.matcher(email).matches()) return "Formato de email inválido.";
        if (telefono != null && !telefono.isEmpty()) {
            // permitir + al inicio opcional y dígitos, pero para simplificar pedimos solo dígitos
            if (!telefono.matches("\\d+")) return "Teléfono inválido: solo dígitos permitidos.";
            if (telefono.length() < 7 || telefono.length() > 15) return "Teléfono inválido: debe tener entre 7 y 15 dígitos.";
        }
        return null; // ok
    }

    private void limpiarCampos() {
        txtNom.setText(""); txtApe.setText(""); txtEmail.setText(""); txtTel.setText("");
